...
export const ChatProvider = ({ children }) => {
  ...
  // 3️⃣ Send Message (With Weather, Smart Generation & FLIGHTS)
  const sendMessage = async (text) => {
    if (!text.trim()) return;
    ...
    try {
      ...
      // B. Check for JSON (Trip Plan OR Flight Search)
      const jsonMatch = replyText.match(/\{[\s\S]*\}/);
      ...
      if (jsonMatch) {
        try {
          ...
          if (parsedData.isFlightSearch) {
            const { origin, destination, date } = parsedData.flightDetails;

            // Construct a Google Flights Search URL
            const flightUrl = `https://www.google.com/search?q=flights+from+${origin}+to+${destination}+on+${date}`;

            replyText = `✈️ **Looking for flights?** \n\nI found some options from **${origin}** to **${destination}** for **${date}**.\n\n👉 [Click here to view Prices & Book](${flightUrl})`;
          }

          // --- CASE 2: TRIP PLAN ---
          else if (parsedData.isTripPlan) {
            const location = parsedData.userSelection.location;
